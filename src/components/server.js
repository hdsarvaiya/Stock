  ...
  try {
    ...
    const stockDataPromises = symbols.map(async (symbol) => {
      try {
        ...
        return { symbol, error: "Failed to fetch data" }; // Return partial data in case of error
      }
    });

    const stockData = await Promise.all(stockDataPromises);

    // Filter out any entries with an error
    const formattedData = stockData.filter((data) => !data.error);

    // If no valid data was returned, send an error message
